...
class Kitti2DBox(_BaseDataset):
    ...
    def __init__(self, config=None):
        ...
        # print(self.config['SEQ_MAP_DIR'], seqmap_name)
        seqmap_file = os.path.join(self.config['SEQ_MAP_DIR'], seqmap_name)
                ...
                if len(row) >= 4:
                    ...
                    if not self.data_is_zipped:
                        ...
                        if not os.path.isfile(curr_file):
                            raise TrackEvalException('GT file not found: ' + os.path.basename(curr_file) + curr_file)
        ...
        if self.config['TRACKERS_TO_EVAL'] is None:
            self.tracker_list = natsorted(os.listdir(self.tracker_fol))
        else:
            self.tracker_list = natsorted(self.config['TRACKERS_TO_EVAL'])
        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    print(curr_file, "xxxxxxxxxxxxxxxxxxxxxxxxxxxx")
                    if not os.path.isfile(curr_file):
                        raise TrackEvalException(
                            'Tracker file not found: ' + curr_file + "|" + self.tracker_fol + "|" + tracker)
