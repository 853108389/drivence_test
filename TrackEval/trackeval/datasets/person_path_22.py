...
class PersonPath22(_BaseDataset):
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            if self.do_preproc and self.benchmark != 'MOT15' and (
                    gt_ids.shape[0] > 0 or len(crowd_ignore_regions) > 0) and tracker_ids.shape[0] > 0:
                ...
                intersection_with_ignore_region = self._calculate_box_ious(tracker_dets, crowd_ignore_regions,
                                                                           box_format='xywh', do_ioa=True)
                is_within_crowd_ignore_region = np.any(intersection_with_ignore_region > 0.95 + np.finfo('float').eps,
                                                       axis=1)
