...
def create_train_sample_data(input_root, output_root, TRAIN_SEQ_ID, VALID_SEQ_ID, init_or_clear_dirs=True,
                             only_labels=False, add_dt=False, fill_blank_file=False):
    ...
    for seq in range(21):
        seq = '%04d' % seq
        if seq not in TRAIN_SEQ_ID and seq not in VALID_SEQ_ID:
            continue
        ...
        if not os.path.exists(tracking_image):
            continue
        else:
            print(seq, "seq")
        ...
        for frame in frames:
            ...
            with open(os.path.join(res_label, f'{sample_str}.txt'), 'w+') as f:
                temp = []
                temp_d = []
                for s in label_dict[frame]:
                    if "DontCare" in s:
                        temp_d.append(s)
                    else:
                        temp.append(s)
                f.writelines(temp)
                if len(temp) < 2 and fill_blank_file:
                    lines = [
                        "Pedestrian 0 0 -2.523309 1106.137292 166.576807 1204.470628 323.876144 1.714062 0.767881 0.972283 6.301919 1.652419 8.455685 -1.900245 -1\n",
                        "Pedestrian 0 0 -2.523309 1106.137292 166.576807 1204.470628 323.876144 1.714062 0.767881 0.972283 6.301919 1.652419 8.455685 -1.900245 -1"
                    ]
                    f.writelines(lines)
    ...
    with open(os.path.join(res_training, 'seq2sample.txt'), 'w+') as f:
        for seq in range(21):
            seq = '%04d' % seq
            if seq not in seq_to_sample.keys():
                continue
            f.write(f'{seq} ')
